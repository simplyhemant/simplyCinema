...
public class MovieController {
        ...
        @Operation(summary = "Get Movie By ID", description = "Fetch movie details using movie ID")
        @GetMapping("/{movieId}")
        public ResponseEntity<MovieDto> getMovieById(@PathVariable(name = "movieId") Long movieId) {
                log.debug("Fetching movie with ID: {}", movieId);
                MovieDto movieDto = movieService.getMovieById(movieId);
                return ResponseEntity.ok(movieDto);
        }

        @Operation(summary = "Get All Movies", description = "Fetch paginated list of all movies")
        @GetMapping("/all")
        ...
        @Operation(summary = "Search Movies", description = "Search movies by keyword with pagination")
        @GetMapping("/search")
        public ResponseEntity<Page<MovieDto>> searchMovie(@RequestParam(name = "keyword") String keyword,
                        @RequestParam(name = "pageNo", defaultValue = "0") Integer pageNo,
                        @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize) {
